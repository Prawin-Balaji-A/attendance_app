    ...
    with camera_lock:
        if camera is None:
            try:
                ...
                config = temp_camera.create_video_configuration(
                    main={
...
def auto_brighten_frame(frame):
    # Apply CLAHE (Contrast Limited Adaptive Histogram Equalization)
    # This acts as professional software HDR, bringing out shadows without blowing out highlights
...
def capture_frame():
    ...
    frame = auto_brighten_frame(frame)
    return frame
...
class FaceAI:
    ...
    def extract_multiple_features(self, frame, face):
        features = []
        with ai_lock:
            aligned_face = self.recognizer.alignCrop(frame, face)
            
            # 1. Original
            features.append(self.recognizer.feature(aligned_face))
            
            # 2. Darken (simulates shadow)
            dark = cv2.convertScaleAbs(aligned_face, alpha=0.5, beta=0)
            features.append(self.recognizer.feature(dark))
            
            # 3. Brighten (simulates glare/sunlight)
            bright = cv2.convertScaleAbs(aligned_face, alpha=1.5, beta=30)
            features.append(self.recognizer.feature(bright))
            
            # 4. High contrast (simulates harsh directional light)
            contrast = cv2.convertScaleAbs(aligned_face, alpha=1.5, beta=-30)
            features.append(self.recognizer.feature(contrast))
            
            # 5. Gamma correction (shadow lift)
            gamma = 0.5
            invGamma = 1.0 / gamma
            table = np.array([((i / 255.0) ** invGamma) * 255 for i in np.arange(0, 256)]).astype("uint8")
            shadow_lift = cv2.LUT(aligned_face, table)
            features.append(self.recognizer.feature(shadow_lift))
...
def draw_latest_boxes(frame):
    ...
    for result in results:
        ...
        if result.get("status") == "too_far":
            color = (150, 150, 150)
            text = "Too Far"
        elif result.get("known") is True:
            color = (0, 255, 0)
...
def live_scan_loop():
    ...
    while live_running:
        try:
            ...
            known_count = len([r for r in results if r["status"] in ["known", "marked", "already_marked"]])
            unknown_count = len([r for r in results if r["status"] == "unknown"])
            too_far_count = len([r for r in results if r["status"] == "too_far"])
